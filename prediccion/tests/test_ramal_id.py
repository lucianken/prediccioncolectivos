...
def sample_ramal_inputs():
    histories = torch.randn(BATCH, N_VEHICLES, SEQ_LEN, 3)
    ...
    return histories, history_mask, vehicle_mask


def test_per_vehicle_encoder_output_shape(sample_ramal_inputs):
    """(batch=2, n_vehicles=5, 40, 3) → (2, 5, 128)."""
...
def test_cross_fleet_transformer_output_shape():
    """(2, 5, 128) → (2, 5, 128)."""
    x = torch.randn(BATCH, N_VEHICLES, 128)
...
def test_ramal_id_full_forward(sample_ramal_inputs):
    ...
    histories, history_mask, vehicle_mask = sample_ramal_inputs
    model = RamalIdModel(n_ramales=N_RAMALES)
    out = model(histories, history_mask, vehicle_mask)
...
def test_ramal_id_output_is_raw_logits(sample_ramal_inputs):
    ...
    model = RamalIdModel(n_ramales=N_RAMALES)
    out = model(histories, history_mask, vehicle_mask)
    # Logits pueden ser negativos o > 1
